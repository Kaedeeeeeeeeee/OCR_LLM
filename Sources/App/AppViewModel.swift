...
final class AppViewModel: ObservableObject {
    ...
    private func handleSelection(_ rectOpt: CGRect?) async {
        ...
        do {
            var captured: Data?
            if config.useNativeScreencapture {
                // Native interactive selection; returns PNG data or nil if canceled
                captured = try await NativeScreencapture.captureSelectionToPNG()
                if captured == nil { return }
            } else {
                guard let rect = rectOpt else { return }
                // Ensure overlay window is fully dismissed before capturing
                try? await Task.sleep(nanoseconds: 150_000_000)
                let img = try ScreenCapture.captureSelectedRegion(rectInScreenSpace: rect)
                captured = ImageCompression.downscaleAndEncodePNGOrJPEG(img, maxDimension: config.imageMaxDimension, jpegQuality: config.jpegQuality)
                if captured == nil { return }
            }
